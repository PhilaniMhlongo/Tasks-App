(function() {
    ...
    function Game() {
      ...
      async function setAlphabet() {
        ...
        const response = await fetch(`/api/game/library/${G.level}`);
        G.wordLibrary = await response.json();
      }
      ...
      async function updateProgress() {
        const progress = {
          killCount: G.kill_count,
          hitCount: G.hit_count,
          fireCount: G.fire_count,
          passedCount: G.passed_count,
          weapons: G.weapons,
          problematicSequences: G.problematic,
          gameOver: !G.you_arent_dead
        };
        
        await fetch(`/api/game/session/${G.session.id}`, {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
